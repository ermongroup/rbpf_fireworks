...
TRAINING_SEQUENCES = [0,2,3, 6, 10, 12, 13, 14, 17]
VALIDATION_SEQUENCES = [i for i in range(21) if i not in TRAINING_SEQUENCES]
...
DIRECTORY_OF_ALL_RESULTS = '%scoord_ascent_birthClutter_results' % RBPF_HOME_DIRECTORY
...
def modify_parameter(spec, direction):
    ...
    Outputs:
    - None
    - We will have modified the spec which was passed by reference (since it
    is a dictionary).  We make the following changes to the spec:
        -spec['mod_direction'] = direction
        -modify the specified coordinate ascent parameter as directed
    """
...
class Iterate(FireTaskBase):   
    ...
    def run_task(self, fw_spec):
        fw_spec['orig_param_val'] = fw_spec['coord_ascent_params']
        ...
        inc_spec['inc_param_val'] = inc_spec["coord_ascent_params"]
        fw_spec['inc_param_val'] = inc_spec["coord_ascent_params"]
        ...
        dec_spec['dec_param_val'] = dec_spec["coord_ascent_params"]
        fw_spec['dec_param_val'] = dec_spec["coord_ascent_params"]
...
def inc_parameter_idx(param_idx, spec):
    """
    Inputs:
    ...
    """
    new_idx = (param_idx + 1) % len(spec['coord_ascent_params'])
    return new_idx
