...
def gen_data_1source(params):
    ...
    for time_step in range(params['num_time_steps']):
        ...
        for c in range(clutter_count):
            ...
            x_pos = np.random.uniform(X_MIN, X_MAX)
            y_pos = np.random.uniform(Y_MIN, Y_MAX)
            measurementSet.measurements[-1].val.append(bb_size)
            measurementSet.measurements[-1].widths.append(x_pos)
            measurementSet.measurements[-1].heights.append(y_pos)            
...
class GenData(FireTaskBase):   
 #   _fw_name = "Run RBPF Task"
    def run_task(self, fw_spec):
        ...
        measurementSet.write_measurements_to_KITTI_format("%sgenerated_data/measurements1.txt" % RBPF_HOME_DIRECTORY, fw_spec)
        groundTruthSet.write_measurements_to_KITTI_format("%sgenerated_data/ground_truth1.txt" % RBPF_HOME_DIRECTORY, fw_spec, gt = True)

if __name__ == "__main__":
    spec = {
        'det_names' : ['regionlets'],
        'SCORE_INTERVALS' : {'regionlets': [2]},
